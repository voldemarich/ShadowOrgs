...
public class Organization {
    ...
    public Organization(String string_id, OfflinePlayer owner){ //As the command of console
    ...
    public int getRight(OfflinePlayer player){
        if(isMember(player)){
    ...
    public void setRight(OfflinePlayer player, int right) throws OrganizationException{
        if(isMember(player)) {
    ...
    public void addMember(OfflinePlayer player, int right){
        if(!isMember(player)) members.put(player.getName(), right);
    }



    public void removeMember(OfflinePlayer player) throws OrganizationException{
    ...
    public boolean withdrawFunds(OfflinePlayer player, double amount){
        if(ShadowOrgs.econ.bankBalance(bank).balance >= amount && getRight(player)>=0){
            ...
            ActionBroadcaster.getInstance().tell((Player)player, "§aYou've withdrawn " + amount + " from " + this.bank);
            return true;
        }
        else{
            ActionBroadcaster.getInstance().tell((Player)player, "§cInsufficient rights/funds");
            return false;
        }
    }

    public boolean depositFunds(OfflinePlayer player, double amount){
        if(ShadowOrgs.econ.has(player.getName(), amount)){
            ...
            ActionBroadcaster.getInstance().tell((Player)player, "§aYou've deposited " + amount + " to " + this.bank);
            return true;
        }
        else return false;
    }
